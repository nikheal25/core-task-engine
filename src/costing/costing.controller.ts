...
import { AssetCostResponseDto, CostBreakdownItemDto } from './dto/cost-response.dto';
import { AssetCostResponse, CostBreakdown } from './interfaces/costing.interface';
  ...
  private mapToDto(response: AssetCostResponse): AssetCostResponseDto {
    return {
      ...
      buildCost: {
        ...
        breakdown: this.convertBreakdownArrayToRecord(response.buildCost.breakdown),
      },
      runCost: {
        total: response.runCost.total,
        currency: response.runCost.currency,
        period: response.runCost.period,
        breakdown: this.convertBreakdownArrayToRecord(response.runCost.breakdown),
      },
      estimationDate: response.estimationDate,
    };
  }

  private convertBreakdownArrayToRecord(breakdownArray: CostBreakdown[]): Record<string, CostBreakdownItemDto> {
